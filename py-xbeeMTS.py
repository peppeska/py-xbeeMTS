...
class Xbeefun(gtk.Window):
	...
	def __init__(self):
		...
		self.butt_info = mainGlade.get_widget("imagemenuitem10")
                self.butt_info.connect("activate",self.showInfo)
		self.Info = mainGlade.get_widget("Informazioni")
                ...
                self.butt_at.connect("clicked",self.send_serial_AT)
...
############################################################
	...
	def showInfo(self,widget,data=0):
		self.Info.show_all()

        def read_temp(self,widget):
		try:
	                output=self.xbee.read_ADC()
		except:
	...
	def send_serial_AT(self,widget):
		output=self.xbee.send_AT(self.entry_at.get_text())
		...
		# temp sta per variabile temporanea
                ...
                try:
                  self.connect_Serial(self.port.get_text(), self.baudrate.get_text(), int(self.bytesize.get_text()),parity,int(self.stopbits.get_text()))
...
############################################################
	...
	def connect_Serial(self,port,baudrate,bytesize,parity,stopbits):
		  self.xbee=xbee_lib(port,baudrate,bytesize,parity,stopbits)
